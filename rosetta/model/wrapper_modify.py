        ...
        **kwargs,
        ...
        if seqlen >= 1:
            ...
            for i in range(num_sections):
                ...
                # Check if projection is needed for this section
                needs_projection = False
                if self.base_model_idx in self.projector_dict and kv_cache_index is not None:
                    sharer_mask = kv_cache_index[i][0][0][0].item()
                    needs_projection = sharer_mask > 0
                
                # Calculate source model KV cache if needed for projection OR if not the last section
                if needs_projection or i != num_sections - 1:
                    logger.debug(f"Processing source models for kv-cache (needs_projection={needs_projection}, i={i}, num_sections={num_sections})")
    ...
    def speculative_generate(
        ...
        **kwargs,
        ...
        Args:
            ...
            fuse_kv: Whether to fuse target KV cache into draft (deprecated, use prefill_fusion and decode_fusion)
            prefill_fusion: Whether to use C2C fusion during prefill stage
            decode_fusion: Whether to fuse target KV cache into draft during decode stage
		...
		# init方法
        # Determine fusion settings: if fuse_kv=False, disable all fusion
        # Otherwise, use prefill_fusion and decode_fusion parameters
        if not fuse_kv:
            # No fusion at all (baseline)
            decoder = SpeculativeDecoderNoKV(
                ...
                gamma=gamma,
                decode_fusion=False,
                prefill_fusion=None,  # No wrapper needed for baseline
            )
        else:
            # Use the specified fusion settings
            decoder = SpeculativeDecoder(
                ...
                prefill_fusion=self if prefill_fusion else None,  # Pass wrapper only if prefill_fusion is True
            )
