...
# Set chat template for llama-160m
...
print(f"Base: {slm_num_layers} layers, dim={base_dim}")
print(f"Teacher: {llm_num_layers} layers, dim={teacher_dim}")
...
samples = [full_dataset[i] for i in range(2)]
batch = collator(samples)

print(f"Batch keys: {batch.keys()}")
print(f"Input IDs shape: {batch['input_ids'].shape}")
print(f"Labels shape: {batch['labels'].shape}")
print(f"KV cache index sections: {len(batch['kv_cache_index'])}")
for i, kv_idx in enumerate(batch['kv_cache_index']):
    print(f"  Section {i}: shape={kv_idx.shape}, first value={kv_idx[0,0,0]}")
...
if outputs.loss.requires_grad:
    print("\n✓ Loss has gradients, trying backward...")
    try:
        outputs.loss.backward()
        print("✓ Backward successful!")
        
        # Check gradients
        for i, proj in enumerate(rosetta_model.projector_list[:3]):  # Check first 3
            n_params = sum(1 for _ in proj.parameters())
            n_grads = sum(1 for p in proj.parameters() if p.grad is not None)
            print(f"Projector {i}: {n_grads}/{n_params} parameters have gradients")
